...
from utils import *
from crawler321._legacy.database import *
'''
    crawler 启动器
'''
class RunCrawler:
    ...
    def run(
        ...
        **kwargs
        ...
        try:
            ...
            code = 200
            if isinstance(extraction_strategy, MediaExtraction):
                if is_media_type(url):
                    content = self.crawler_strategy.crawl(
                        url=url,
                        proxy=proxy,
                        content_type="media"
                    )
                    extraction_strategy.next_run(url, content)
                    return ''
            def get_html_strategy():
                global code
                if isinstance(extraction_strategy, XPathExtraction):
                    html = extraction_strategy.execute(
                        url, proxy, kwargs.get("actions")
                    )
                else:
                    html, code= self.crawler_strategy.crawl(
                        url=url, 
                        method=method,
            ...
            extraction_result = extraction_strategy.extract(
                url=url, 
                html=html, 
                crawler=self.crawler_strategy,
                proxy=proxy, 
                encoding=encoding
            )
            ...
            if self.report:
                ...
                insert_data(
                    ...
                    html_result.model_dump_json(exclude_none=True), code, t4-t3
                )
